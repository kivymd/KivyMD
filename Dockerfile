...
RUN echo '-----Python 3 ----' && cd demos/kitchen_sink/bin/python3/ && time buildozer android debug || echo "Fix build hello world" \
    && cp -v /home/user/hostcwd/demos/kitchen_sink/bin/python3/kitchen_sink-0_1_3-debug.apk py3-kitchen_sink-0_1_3-debug.apk \
    && /bin/true

RUN echo '-----Python 2 -----' && cd demos/kitchen_sink/bin/python2/ && time buildozer android   || echo "Fix build hello world" \ 
    && cp -v /home/user/hostcwd/demos/kitchen_sink/bin/python2/KivyMDKitchenSink-0.1.3.apk ~/py2-KivyMDKitchenSink-0.1.3.apk && date && /bin/true
